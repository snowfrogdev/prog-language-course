...
local function nodeDecimal(num)
...
local scientificPart = lpeg.V "scientificPart"
local integer = lpeg.V "integer"
local sign = lpeg.V "sign"
...
local grammar = lpeg.P { "exp",
  ...
  opU = lpeg.C("-") * space,
  primary = identifier + number + OP * exp * CP,
  OP = "(" * space,
  CP = ")" * space,
  identifier = ((lpeg.R("az", "AZ") + lpeg.S("_")) * (lpeg.R("az", "AZ", "09") + lpeg.S("_")) ^ 0) / nodeVar * space,
  number = decimalNumber + hexNumber,
  decimalNumber = ((integer * "." * integer * scientificPart ^ 0) + (integer * scientificPart ^ 0)) / nodeDecimal * space,
  scientificPart = lpeg.S("eE") * sign ^ 0 * integer,
  integer = lpeg.R("09") ^ 1,
  sign = lpeg.S("+-"),
